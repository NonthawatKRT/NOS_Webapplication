...
<body>
    ...
    if ($userrole == 'Customer') {
        ...
        // ดึงข้อมูลแพ็คเกจของพนักงาน
        $sql = "SELECT p.PolicyName, p.imageName, ep.PaymentStatus, ep.EnrollmentDate, ep.PolicyID
        FROM employeepolicy ep
        JOIN policy p ON ep.PolicyID = p.PolicyID
        WHERE ep.EmployeeID = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            die("SQL Error: " . $conn->error);
        ...
        // Get customer information for the employee
        // Step 1: Get SalesID from sales table
        $sql = "SELECT SalesID FROM sales WHERE EmployeeID = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            die("SQL Error: " . $conn->error);
        }
        ...
        if (!$SaleID) {
            die("No SalesID found for this EmployeeID.");
        }

        // Step 2: Get all CustomerID and PolicyID from salescustomer table
        $sql = "SELECT CustomerID, PolicyID FROM salescustomer WHERE SalesID = ?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            die("SQL Error: " . $conn->error);
        }
        ...
        // echo "Customer IDs and Policies: <pre>";
        // print_r($customers);
        // echo "</pre>";

        // Step 3: Query each customer's details
        $sql = "SELECT 
            c.CustomerID,
            c.Email, 
            ...
            cp.EnrollmentDate
        FROM customer c
        INNER JOIN salescustomer sc ON c.CustomerID = sc.CustomerID
        INNER JOIN customerpolicy cp ON sc.PolicyID = cp.PolicyID AND cp.CustomerID = sc.CustomerID
        INNER JOIN policy p ON cp.PolicyID = p.PolicyID
        WHERE sc.CustomerID = ? AND sc.PolicyID = ?";
        ...
        foreach ($customers as $customer) {
            ...
            $stmt->bind_param("si", $CustomerID, $PolicyID); // Adjust data types
            if ($stmt->execute()) {
                $result = $stmt->get_result();
                if ($result->num_rows > 0) {
                    ...
                    echo "No details found for CustomerID: $CustomerID and PolicyID: $PolicyID <br>";
                }
            } else {
    ...
    <div class="spacer">
        ...
        <div class="secondmainsection">
            <?php if ($userrole === 'Customer'): ?>
                <div class="packagesection">
                    <div class="sectiontittle">
                        <h2>กรมธรรณ์ของฉัน</h2>
                        <hr>
                    </div>
            ...
            <?php if ($userrole === 'Employee'): ?>
                ...
                <div class="customersection">
                    ...
                    <div class="customerinfo fade-up">
                        <?php if (!empty($customerDetails)): ?>
                            <?php foreach ($customerDetails as $customer): ?>
                                <div class="card">
                                    <?php
                                    // Dynamically determine the profile picture path
                                    $profilePicturePath = "userprofiles/" . $customer['Email'] . ".png";
                                    ...
                                    ?>
                                    <!-- Profile Picture -->
                                    <div class="cardprofilepiccontainer">
                                        <div class="cardprofileframe">
                                            <img src="<?php echo htmlspecialchars($profilePicturePath); ?>" alt="User Profile Picture" class="user-profile-pic">
                                        </div>
                                    </div>
                                    ...
                                    <a href="#" id="userprofileinfo-link" data-email="<?php echo htmlspecialchars($customer['Email']); ?>">
                                        ...
                                        <div class="customercontent">
                                            ...
                                            <div class="customerdetails">
                                                <h3><?php echo htmlspecialchars($customer['CustomerName']); ?></h3>
                                                <p><strong>Package:</strong> <?php echo htmlspecialchars($customer['PolicyName']); ?></p>
                                            </div>
                                            <div class="customerdetails">
                                                <div>
                                                    <p class="payment-status <?php echo strtolower($customer['PaymentStatus']); ?>">
                                                        <?php echo $customer['PaymentStatus'] === 'Paid' ? 'Payment Completed' : 'Pending Payment'; ?>
                                                </div>
                                                <p><strong>Enrollment Date:</strong> <?php echo htmlspecialchars($customer['EnrollmentDate']); ?></p>
                                            </div>
                                            </p>
